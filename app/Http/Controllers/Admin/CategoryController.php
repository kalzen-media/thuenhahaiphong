...
class CategoryController extends Controller
{
    ...
    {
        $records = DB::table('chuyen_muc')->get();
        foreach ($records as $record)
        {
            DB::beginTransaction();
            try {
            $input['id']= $record->Id;
            $input['name']= $record->TieuDe;
            $input['description'] = $record->MoTaNgan;
            $input['slug'] = $record->Slug;
            $category = Category::create($input);
            $category->tags()->createMany(collect(explode(', ',$record->TuKhoa))->map(function($item){return ['name'=>$item];}));
            
                $category->image()->updateOrCreate(['url' => $record->Thumbnail]);
